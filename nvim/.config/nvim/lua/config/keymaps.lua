...
set("n", "<leader>fp", function()
	require("config.search").find_files(get_project_root())
end, { desc = "Find Files In Closest Git Root" })
set("n", "<leader>fr", function()
	require("config.search").find_files(vim.fn.expand("~/src/http"))
end, { desc = "Find HTTP Files in ~/src/http" })
set("n", "<leader>ff", function()
	require("config.search").find_files(vim.fn.getcwd())
end, { desc = "Find Files Working Directory" })
set("n", "<leader>fg", function()
	require("config.search").live_grep(vim.fn.getcwd())
end, { desc = "Live Grep" })
set("n", "<leader>fz", function()
	require("config.search").live_grep(vim.fn.getcwd(), { grep = { modes = { "fuzzy", "plain" } } })
end, { desc = "Fuzzy Grep" })
set("n", "<leader>fc", function()
	require("config.search").live_grep(vim.fn.getcwd(), { query = vim.fn.expand("<cword>") })
...
set("n", "<leader>fs", function()
	...
	if ok then
		...
		vim.notify("fff.nvim is not available", vim.log.levels.WARN)
	end
